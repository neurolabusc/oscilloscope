...
begin
  if Timer1.Tag = 1 then WriteReturnToKeyMode; //close previous device if currently connected
  ...
  case SerialBuffer[position+1] of
       ...
       kCmd2KeyDown : begin //Arduino is reporting key press mapping
             if (SerialBuffer[position+2] = 0) then begin
                Timer1.Tag := 1; //this allows us to detect if a supported device is connected
  ...
  for c :=1 to kKeyNum do begin
        if odd(bits shr (c-1)) then
           if (c > kKeyNumDigital) then
            ...
            result := result + '0';
        if c < kKeyNum then result := result + chr(9);
  end;
end;
...
begin
  ...
  //TextOutput.lines.add ( inttostr(keyBits)+chr(9)+ inttostr(uSec)+keyStr(keyBits));
  //Format('Padded decimal    = <%7d>', [1234])
  TextOutput.lines.add ( Format('%8d',[uSec])+chr(9)+ Format('%4d',[keyBits])+chr(9)+keyStr(keyBits));
...
begin
     ...
     TextOutput.lines.add('#uSec'+chr(9)+'#Keys');
end;
...
begin
  if Timer1.Tag = 1 then WriteReturnToKeyMode; //if currently connected
  ser.Free;
  SerialBuffer := nil;
end;
