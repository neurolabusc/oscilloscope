...
begin
  showmessage('Alpha version: November 2012 - for testing only ');
end;
...
var
  ...
  packetBytes,sampleNumber, samples,is16bit, analogChannels, digitalByte, len,pos, c, checkSumStored, checkSum: integer;
begin
 ...
 len := length(gMainPrefs.SerialBuffer);
  if (len < packetBytes) then exit; //no new packets
    ...
    while ((len-pos) >= packetBytes) do begin
         ...
         analogChannels  := serialBytes[0] and $0f;
         if ((checkSum = checkSumStored) and (analogChannels = gMainPrefs.AnalogChannels)) then begin
            digitalByte := serialBytes[2];
            is16bit  := (serialBytes[0] shr 6) and $01;
            if (not gMainPrefs.Recording) then begin
                  gMainPrefs.AnalogChannels := analogChannels;
                  gMainPrefs.Precision16bit := odd(is16bit);
                  MainForm.initTimer.tag := MainForm.initTimer.tag + 1;
             end else if (gMainPrefs.EEG.maxsamples > 0) then begin
                  for c := 0 to (analogChannels-1) do
                      gMainPrefs.EEG.samples[c][gMainPrefs.EEG.samplesacquired] :=  (serialBytes[3+(c*2)]  shl 8) +serialBytes[4+(c*2)] ;
                  (*if (analogChannels > 1) then
                     gMainPrefs.EEG.samples[1][gMainPrefs.EEG.samplesacquired] := channelB;
                  if (analogChannels > 2) then
                     gMainPrefs.EEG.samples[2][gMainPrefs.EEG.samplesacquired] := channelC;  *)
                  if (gMainPrefs.DigitalChannel) then
                     gMainPrefs.EEG.samples[analogChannels][gMainPrefs.EEG.samplesacquired] := digitalByte;
                  ...
                  if (gMainPrefs.DigitalCurrent <> digitalByte) then begin
                        if (digitalByte > gMainPrefs.DigitalCurrent) then begin //value increased - event onset detected
                           gMainPrefs.LastDigitalOnsetSample:= gMainPrefs.EEG.samplesacquired;
                           gMainPrefs.LastDigitalOnset:= digitalByte;
                           ...
                           AddVMRK(digitalByte - gMainPrefs.DigitalCurrent, gMainPrefs.EEG.samplesacquired, gVMRK);

                        end; //if digitalByte increase
                        gMainPrefs.DigitalCurrent := digitalByte;
...
begin
  ...
  if ((gMainPrefs.SerialBuffer[position] = kCmd1Set) and   (gMainPrefs.SerialBuffer[position+1] in [kCmd2Mode,kCmd2KeyDown,kCmd2KeyUp,kCmd2KeyTrigger,kCmd2OscHz,kCmd2OscChannels])) then begin
...
begin
     ...
     if (gMainPrefs.sampleRateHzObs = 0) then begin
        gMainPrefs.sampleRateHzObs := round(initTimer.Tag/(initTimer.interval/1000) );
...
begin
     ...
     setlength(gMainPrefs.SerialBuffer,0);
     //WriteCmd(true,kCmd2OscChannels,0,3); //set number of channels
     //WriteCmd(true,kCmd2OscHz,0,250); //set sampling rate
     //if (gMainPrefs.AnalogChannels > kMaxChannels) then gMainPrefs.AnalogChannels :=  kMaxChannels;
     //if (gMainPrefs.AnalogChannels < 1) then gMainPrefs.AnalogChannels :=  1;
     ...
     WriteCmd(true,kCmd2Mode,kCmd34ModeOsc,kCmd34ModeOsc); //set to Osc Mode
     MainForm.StatusBar1.Panels[2].Text:='Hz: '+inttostr(gMainPrefs.sampleRateHz);
     initTimer.Tag := 0; //detect samples...
